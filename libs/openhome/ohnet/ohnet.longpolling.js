...
ohnet.longpolling = function (clientId,nodeUdn,options,host) {
	this.clientId = clientId;
	this.proxy =  new CpProxyOpenhomeOrgSubscriptionLongPoll1(nodeUdn,host);
	this.pollingStarted = false;
	var defaults = {
   			debug : false,
   			subscriptionTimeoutSeconds: 10,
...
ohnet.longpolling.prototype.subscribe = function (service,firstSubscription) {
	...
	this.proxy.Subscribe(this.clientId,service.udn,service.serviceName,this.subscriptionTimeoutSeconds,
		...
		function()
		{
			if (_this.onReceiveError) { _this.onReceiveError(); _this.isSubscribe = true;}
		}
	);
};

ohnet.longpolling.prototype.getLastReviceDate = function(){
	return this.lastReciveDate;
};

ohnet.longpolling.prototype.hasSubscribe = function(){
...
*/
ohnet.longpolling.prototype.renew = function (subscriptionId, success, failed) {
	var _this = this;
...
ohnet.longpolling.prototype.propertyUpdate = function () {
	...
	if(this.pollingStarted)
	{
		...
		this.proxy.GetPropertyUpdates(this.clientId, function(results) {
			_this.lastReciveDate = new Date();
            if (_this.debug) { console.log('<< GetPropertyUpdates'); }
            if (_this.onReceiveSuccess) { _this.onReceiveSuccess(); }
           if(results.Updates != null){ 
            _this.onReceivePropertyUpdate($.parseXML(results.Updates));
           }
           /**
		...
		function()
		{
			console.log('Long poll failed, retrying...');
...
* 是否启动 polling
*/
ohnet.longpolling.prototype.isStarted = function(){
	return !this.isSubscribe || this.pollingStarted;
}